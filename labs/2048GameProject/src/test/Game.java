...
public class Game extends JPanel implements KeyListener
{
    ...
    public void paint( Graphics g )
    {
        ...
        g2.drawString( "Счёт: " + game.getScore(),
                200 - 4 * String.valueOf( game.getScore() ).length(),
                40 );
        g2.drawString( "Наибольшая плитка: " + game.getHighTile(),
                280 - 4 * String.valueOf( game.getHighTile() ).length(),
                40 );
        g2.drawString( "Нажмите Enter для начала игры", 210, 315 );
        g2.drawString( "Для управления используйте стрелки или wasd", 180, 335 );
        if ( game.blackOut() )
        {
            g2.drawString( "Нажмите Enter для перезапуска игры", 200, 355 );
        }
        ...
        if ( game.gameOver() )
        {
            ...
            for ( int i = 0; i < 4; i++ )
            {
                for ( int j = 0; j < 4; j++ )
                {
                    ...
                    g2.fillRoundRect( j * 60 + 150, i * 60 + 60, 50, 50, 20, 5 );
